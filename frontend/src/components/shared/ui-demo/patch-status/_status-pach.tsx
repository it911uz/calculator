...
import { TApartmentStatus, IApartment } from "@/types/apartment.types"; // IApartment'ni import qiling
import {
  Select,
  SelectContent,
  SelectItem,
...
const statusConfig: Record<TApartmentStatus, { label: string; className: string }> = {
...
export const StatusUpdateCell: React.FC<StatusUpdateCellProps> = ({ apartment }) => {
    ...
    >
      <SelectTrigger 
        className={`h-6 w-32 border-none shadow-none focus:ring-0 px-3 py-1 rounded-md text-sm font-medium ${
          statusConfig[apartment.status as TApartmentStatus]?.className
        }`}
      >
        {isPending ? <Loader2 className="h-4 w-4 animate-spin mx-auto" /> : <SelectValue />}
      </SelectTrigger>
      <SelectContent>
        {(Object.keys(statusConfig) as TApartmentStatus[]).map((status) => (
