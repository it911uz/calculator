  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-96">
        <DialogHeader>
          <DialogTitle className="text-xl font-bold">Редактировать роль</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="space-y-3">
              ...
              {permsLoading ? (
                ...
                <div className="space-y-2.5">
                  {allPermissions?.map(perm => (
                    <div key={perm.id} className="flex items-center gap-2 hover:bg-gray-50 p-1 rounded transition-colors">
                      <Checkbox 
                        id={`edit-perm-${perm.id}`} 
                        checked={selectedIds.includes(perm.id)}
                        onCheckedChange={(checked: boolean) => handleToggle(perm.id, checked)}
                      />
                      <label htmlFor={`edit-perm-${perm.id}`} className="text-xs cursor-pointer select-none flex-1">
                        {perm.codename}
                      </label>
                    </div>
                  ))}
            ...
            <p className="text-[11px] text-muted-foreground font-medium">
              Выбрано: <span className="text-indigo-900">{selectedIds.length}</span> из {allPermissions?.length || 0}
            </p>
          </div>
        </div>
        ...
        <DialogFooter className="gap-2">
          ...
          <Button onClick={handleSave} disabled={isPending || !name.trim()} className="bg-indigo-900 text-white min-w-24 hover:bg-indigo-800">
            {isPending ? <Loader2 className="animate-spin h-4 w-4" /> : "Сохранить"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
