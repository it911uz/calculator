		...
		<Table>
			...
			<TableBody>
				{users?.map((user, i) => {
					...
					return (
						<TableRow key={user.id} className={isMe ? "bg-gray-50/50" : ""}>
							<TableCell className={isMe ? "opacity-50" : ""}>
								{i + 1}
							</TableCell>
							<TableCell
								className={`font-medium ${isMe ? "text-[#282964] disabled:cursor-not-allowed disabled:opacity-50" : ""}`}
							>
								{user.username}{" "}
								{isMe && (
