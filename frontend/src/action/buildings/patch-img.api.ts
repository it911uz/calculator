...
export async function updateBuildingImage(
  ...
  try {
    ...
    if (!auth?.access) {
      result._meta = { status: 401, error: "Sessiya muddati tugagan", reason: "TOKEN" };
      return result;
    }
    ...
    const res = await fetch(`${ENV.BASE_URL}/buildings/${buildingId}/image/`, {
      method: "PATCH", 
      headers: {
    ...
    result._meta = {
      status: 500,
      error: error instanceof Error ? error.message : "Server bilan bog'lanishda xatolik",
      reason: "UNKNOWN",
    };
    return result;
  }
