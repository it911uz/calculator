...
export async function deleteRole(
  ...
  const result: SafeObject<{ success: boolean }> = { data: { success: false } } as SafeObject<{ success: boolean }>;
  const queryStr = createSearchParams(params).toString();
  try {
    const authData = await getAuthData();

    if (!authData?.access) {
      result._meta = { status: 401, error: "Неавторизованный токен", reason: "TOKEN" };
      return result;
    }

    const res = await fetch(`${ENV.PUBLIC_API_URL}/roles/${id}/${queryStr ? `?${queryStr}` : ""}`, {
      method: "DELETE",
      headers: {
