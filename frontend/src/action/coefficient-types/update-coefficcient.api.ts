...
export async function updateCoefficientType(
    ...
    try {
        ...
        const res = await fetch(
            `${ENV.PUBLIC_API_URL}/coefficient-types/${coefficientTypeId}/${searchParams ? `?${searchParams}` : ""}`,
            {
                method: "PATCH",
                headers: {
        ...
        if (!res.ok) {
            ...
            result._meta = {
                status: res.status,
                error:
                    errorData.detail || `Ошибка при обновлении: ${res.status}`,
                reason: "HTTP",
            };
            return result;
        }
