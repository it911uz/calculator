...
import type { IPermission, IPermissionFilters } from "@/types/permissions.types";
import type { SafeArray } from "@/types/safe-response.types";

export async function getPermissions(params: IPermissionFilters): Promise<SafeArray<IPermission>> {
  ...
  try {
    ...
    const res = await fetch(`${ENV.PUBLIC_API_URL}/permissions/${queryStr ? `?${queryStr}` : ""}`, {
      method: "GET",
      headers: {
