...
export async function getAuthMe(): Promise<SafeObject<IUserMe>> {
  ...
  try {
    ...
    if (!auth?.access) {
      result._meta = { status: 401, error: "Avtorizatsiya zarur", reason: "TOKEN" };
      return result;
    }

    const res = await fetch(`${ENV.BASE_URL}/auth/me`, {
    ...
    if (!res.ok) {
      result._meta = { status: res.status, error: data?.detail, reason: "HTTP" };
      return result;
    }
    ...
    result._meta = { status: 500, error: "Noma'lum xato", reason: "UNKNOWN" };
    return result;
  }
}